...
public class AI : MonoBehaviourPunCallbacks
{
    ...
    private bool isCourutineRunning = false;
    ...
    private void Update()
    {
        ...
        if (isTower)
        {
            if (playerInAttackRange)
            {
                ...
                SetTargetSign(GetClosestEnemy().gameObject);
            }
            else
            {
    ...
    private void SearchWalkPoint()
    {
        ...
        if (enemy)
        {
            NavMesh.CalculatePath(transform.position, enemy.transform.position, NavMesh.AllAreas, path);
        }
        agent.SetPath(path);
    ...
    private void SetTargetSign(GameObject targetGo)
    {
        if (!isCourutineRunning && targetGo.GetComponent<TankScripts.TankShooting>())
        {
            targetGo.GetComponent<TankScripts.TankShooting>().SetTowerTargeting();
            StartCoroutine(SetSign(targetGo));
        }
    }

    private IEnumerator SetSign(GameObject targetGo)
    {
        isCourutineRunning = true;
        yield return new WaitForSeconds(1f);
        if (targetGo)
        {
            targetGo.GetComponent<TankScripts.TankShooting>().DisableTowerTargeting();
        }
